function [Peak,SPeak] = ExtractPeaksForBeginDates(CoefCOI,time,Sgn,dtb2,mts,varargin)
% Extracting first repeating peaks in mean periodic bands for different years
...
% 2) filter_sign: Set a filter on sign of coefficients (negative), no value needed for this argument
...
for j = 1:no_dtb
    ...
    if height(idx)>length(unique(year(idx.ts))) % More than one peak per year
        ...
        for i = year(idx.ts(1)):year(idx.ts(end))
            ...
            if height(vals)>1
                if filter_sign
                    vals = vals(vals.wave<0,:);% Delete positive cofficients
                end
                vals = vals(vals.pk>mean(vals.pk),:); % Delete values above average
                idx(idx.Year==i & idx.ts~=vals.ts(1),:) = []; % Keep only first date above average
            end
        end
    end
